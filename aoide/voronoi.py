...
def match_int(a, b):
    ...
    subb = []
    count = 0
    if (minab > maxab) or (maxab < 0):
        return suba, subb, count

    comrange = numpy.arange(minab, maxab + 1)           # common values
    # bin-borders of common values
    binrange = numpy.arange(minab - 0.5, maxab + 1.5, 1)
    ...
    if count > 0:
        # indices in a for common elements
        suba = numpy.searchsorted(a, comrange[common])
        # indices in b for common elements
        subb = numpy.searchsorted(b, comrange[common])
...
def wvt_make_neighborlist(x, y):
    ...
    mask = numpy.zeros((len(uniquex) + 2, len(uniquey) + 2),
                       dtype=numpy.int) - 1
    for i in range(npix):
...
def wvt_renumber_binclass(binclass, start_binclass=0, area=[]):
    ...
    if len(area) == 0:
        area = numpy.histogram(binclass, list(
            range(start_binclass, (max(binclass) + 2))))[0]
...
def wvt_check_all_bins(binclass, x, y, xnode, ynode):
    ...
    area = numpy.histogram(binclass, list(range(0, max(binclass) + 2)))[0]
...
def wvt_check_binislands(x, y, binclass, neighborbinclass, xnode, ynode):
    ...
    for k in range(nbins):
        ...
        if numpy.sum(w) > 0:
            neighbornodes = neighbornodes[w]
        if numpy.sum(w) == 1:
            k2 = neighbornodes[0]
            print('Bin', k, 'is enclosed by bin', k2, '| Redistributing bins.')
            ind = numpy.append(ind, numpy.where(binclass == k2)[
                               0])  # add the other bin pixels
            # only search among 2 nodes
            tmp = numpy.array([k, k2], dtype=numpy.int)
            for i in ind:
...
def wvt_calc_binSN(binclass, signal, noise):
    ...
    # if len(area) == 0:
    area = numpy.histogram(binclass, list(range(0, nbins + 1)))[0]
    good = numpy.where(area > 0)[0]    # Check for zero-size Voronoi bins

    # Only loop over nonzero bins
    for i in good:
        # Find subscripts of pixels in bin i
        ind = numpy.where(binclass == i)[0]
...
def show_binclass(x, y, binclass):
    ...
    for i in range(len(x)):
        binclass_arr[y[i], x[i]] = binclass[i]

    plt.imshow(binclass_arr, vmin=0, interpolation='nearest', origin='lower',
...
def wvt_bin_accretion(x, y, signal, noise, targetSN, dens,
    ...
    # will contain bin numberof pixel
    binclass = numpy.zeros(npix, dtype=numpy.int)
    # will contain 1 if bin has been accepted as goos
    good = numpy.zeros(npix, dtype=numpy.int)
    # will contain S/N of the bin
    binSN = numpy.zeros(npix, dtype=numpy.float)
    ...
    SN = dens[currentBin]
    print('First bin starts at X = %i, Y = %i' %
          (x[currentBin], y[currentBin]))
    ...
    for ind in range(1, npix + 1):     # The first bin will be assigned BIN = 1
        ...
        # Here currentBin is still made of one pixel
        binclass[currentBin] = ind
        xbar = x[currentBin]
        ...
        while True:
            ...
            modtargetSN = targetSN - SN * \
                (numpy.sqrt(1. + 1. / binarea) - 1.) / 2.
            ...
            neighbors, exneighbors = wvt_find_neighbors(
                currentBin, neighborlist, neighbors, exneighbors, newmember)
            ...
            k = numpy.argmin((x[neighbors] - xbar)**2 +
                             (y[neighbors] - ybar)**2)
            ...
            xbar, ybar, mass = wvt_addto_weighted_centroid(
                x[newmember], y[newmember], dens[newmember]**2, xbar, ybar, mass)
        ...
        k = numpy.argmin((x[unBinned] - xbar_start)**2 +
                         (y[unBinned] - ybar_start)**2)
        # The bin is initially made of one pixel
        currentBin = numpy.asarray([unBinned[k]], dtype=numpy.int)
...
def wvt_reassign_bad_bins(x, y, binclass, quiet):
    ...
    area = numpy.histogram(binclass, list(range(1, max(binclass) + 2)))[0]
    ...
    for i in range(len(good)):
        # Find subscripts of pixels of each bin
        ind = numpy.where(binclass == good[i] + 1)[0]
...
#        raw_input()
    ...
    for i in range(len(good)):
        # Find subscripts of pixels in each bin
        ind = numpy.where(binclass == good[i])[0]
        xnode[i], ynode[i] = wvt_unweighted_centroid(x[ind], y[ind])
...
def wvt_equal_mass(x, y, signal, noise, targetSN, dens, binclass, neighborlist,
    ...
    if max_iter == 0:
        max_iter = 1000   # number of maximal iterations
    ...
    while diff != 0. and iter <= max_iter:
        ...
        for i in range(npix):
            binclass[i] = wvt_assign_to_bin(
                x[i], y[i], xnode, ynode, binareaSN)
        ...
        for i in good:
            ...
            if gersho == False:     # get actual positions of the nodes
                ...
                xnode[i], ynode[i] = wvt_weighted_centroid(
                    x[ind], y[ind], dens2[ind])

        # Redistribute enclosed bins ("bin islands")
        binclass, area = wvt_check_binislands(
            x, y, binclass, neighborbinclass, xnode, ynode)
        ...
        if quiet == False:
            if iter > 0:
                print(r'Iteration: %3i | Difference: %5.4f %%' % (iter, diff))
            else:
...
def write_binning_results(x, y, signal, noise, outfile_prefix, size, binclass,
    ...
    if (valid_out == 0) or (valid_out == 2):
        ...
        table_file.write(
            '#    X      Y   binNr      signal           noise               sn              distance\n')
        ...
        print('storing bin properties in ' +
              outfile_prefix + '.voronoi.bin.dat')
    ...
    if (valid_out == 1) or (valid_out == 2):
        select_bin = numpy.where(numpy.logical_and(
            binSN >= valid_SN, area < valid_area) == True)[0]
        select_pix = numpy.zeros(0, dtype=numpy.int)
        for i in select_bin:
            select_pix = numpy.append(
                select_pix, numpy.where(binclass == i)[0])

        table_file = open(outfile_prefix + '.voronoi.pixel.valid.dat', 'w')
        table_file.write(
            '#    X      Y   binNr      signal           noise               sn              distance\n')
        print('storing valid binnumbers in ' +
              outfile_prefix + '.voronoi.pixel.dat')
        max_sn_bin = numpy.argmax(signal / noise)
        for i in range(len(select_pix)):
            table_file.write('%6i\t%6i\t%6i\t%16.6e\t%16.6e\t%16.6f\n' % (x[select_pix[i]], y[select_pix[i]], binclass[select_pix[i]] + 1, signal[select_pix[i]],
                                                                          signal[select_pix[i]] / noise[select_pix[i]], numpy.sqrt((x[max_sn_bin] - x[select_pix[i]])**2 + (y[max_sn_bin] - y[select_pix[i]])**2)))
        table_file.close()
        ...
        for i in range(len(select_bin)):
            table_file.write('%d\t%16.6f\t%16.6f\t%16.6f\t%16.6f\t%16.6e\t%16.6e\t%16.6f\t%5i\t%16.6f\t%16.6f\n' % (select_bin[i] + 1, xnode[select_bin[i]], ynode[select_bin[i]], xbar[select_bin[i]], ybar[select_bin[i]], binsignal[
                             select_bin[i]], binnoise[select_bin[i]], binSN[select_bin[i]], area[select_bin[i]], scale[select_bin[i]], numpy.sqrt((xbar[max_sn_bin] - xbar[select_bin[i]])**2 + (ybar[max_sn_bin] - ybar[select_bin[i]])**2)))
        table_file.close()
        ...
        if full_output == True:
            ...
            sn_arr = numpy.zeros(size, dtype=numpy.float)
            for i in range(len(select_pix)):
                bin_arr[y[select_pix[i]], x[select_pix[i]]
                        ] = binclass[select_pix[i]] + 1
                signal_arr[y[select_pix[i]], x[select_pix[i]]
                           ] = binsignal[binclass[select_pix[i]]]
                noise_arr[y[select_pix[i]], x[select_pix[i]]
                          ] = binnoise[binclass[select_pix[i]]]
                sn_arr[y[select_pix[i]], x[select_pix[i]]
                       ] = binSN[binclass[select_pix[i]]]
...
def wvt_binning(x, y, signal, noise, targetSN, dens,
    ...
    if max_area == 0:
        max_area = npix   # maximal bin area
    ...
    noise[numpy.where(noise == 0)] = numpy.min(
        noise[numpy.where(noise > 0)]) * 1e-9

    if (len(dens) != npix):
        dens = numpy.zeros(npix, numpy.float)
        for i in range(npix):
            dens[i] = float(signal[i]) / float(noise[i])
    ...
    xbar, ybar, binsignal, binnoise, binSN = compute_bin_quantities(
        x, y, signal, noise, binclass)
    ...
    print('     Binned pixels:  %i/%i = %.2f%%' %
          (npix - len(w1), npix, float(npix - len(w1)) / float(npix) * 100.))
    print('     Valid bins:     %i/%i = %.2f%%' % (numpy.sum(w4) + numpy.sum(w1),
                                                   len(xnode), float(numpy.sum(w4) + numpy.sum(w1)) / float(len(xnode)) * 100.))
    print('----------------------------------------------------')
    print('S/N statistics for valid binned pixel:')
    print('     target S/N:     %.2f' % (targetSN))
    print('     average S/N:    %.2f' % (averageSN))
    print('     S/N scatter around target S/N:  %.2f %%' %
          (numpy.std((binSN[w4] - targetSN) / targetSN) * 100.))
    print('     S/N scatter around average S/N: %.2f %%' %
          (numpy.std((binSN[w4] - averageSN) / averageSN) * 100.))
    print('----------------------------------------------------')
...
def binning_fits(signalfile, noisefile, targetSN, outfile_prefix, full_output=True,
    ...
    if (noise_mode == 'var'):
        datanoise = numpy.sqrt(datanoise)
    ...
    if valid_SN == 0:
        valid_SN = 0.8 * targetSN
    if valid_area == 0:
        valid_area = 1.2 * max_area
